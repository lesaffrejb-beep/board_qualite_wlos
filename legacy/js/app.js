...
(function () {
    ...
    async function authenticateAdmin() {
        ...
        if (supabase) {
            try {
                ...
                if (!error && data === true) isAuthenticated = true;
            } catch (e) { console.error(e); }
        } else {
            // Fallback local
            if (password === CONFIG.ADMIN_PASSWORD || password === CONFIG.SITE_PASSWORD) isAuthenticated = true;
        }
    ...
    async function authenticateSite() {
        ...
        if (supabase) {
            try {
                ...
                console.error(e);
                // Si erreur RPC, on tente le fallback local (ex: si connexion perdue)
                if (password === CONFIG.SITE_PASSWORD || password === CONFIG.ADMIN_PASSWORD) isAuthenticated = true;
            }
        } else {
            // Fallback
            if (password === CONFIG.SITE_PASSWORD || password === CONFIG.ADMIN_PASSWORD) isAuthenticated = true;
    ...
    async function attemptUnlock() {
        ...
        if (supabase) {
            try {
                const { data } = await supabase.rpc('verify_admin_password', { password_input: password });
                if (data === true) isAuthenticated = true;
            } catch (e) { }
        }
        // Fallback
        if (!isAuthenticated && (password === CONFIG.ADMIN_PASSWORD)) isAuthenticated = true;
